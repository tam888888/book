...
(async () => {
  ...
  let csvVND = new Parser({ fields: ['code', 'floor', 'tradingDate', 'time', 'tradingDate_Time', 'open', 'high', 'low', 'last', 'adLast', 'accumulatedVal', 'accumulatedVol', 'lastVol', 'side'] });
  let csvSSI = new Parser({ fields: ["symbol", 'stockNo', 'price', 'vol', 'accumulatedVol', 'time', 'ref', 'side', 'priceChange', 'priceChangePercent', 'changeType'] });
  ...
  let ssi = await getlistallstockssi();
      ...
      if (x.Code.length < 4) {
        ...
        setTimeout(() => {
          ...
          z.then((ret) => {
            ...
            let data2 = csvSSI.parse(ret.data.leTables);
...
async function getTransVND(symbol) {
  let ok = false;
  let x;
  while (!ok) {
    let a;
    try {
      a = await fetch("https://api-finfo.vndirect.com.vn/v4/stock_intraday_latest?q=code:" + symbol + "&sort=time&size=100000", {
        "headers": {
          "accept": "application/json, text/plain, */*",
          "sec-ch-ua": "\"Chromium\";v=\"92\", \" Not A;Brand\";v=\"99\", \"Google Chrome\";v=\"92\"",
          "sec-ch-ua-mobile": "?0"
...
async function getTransSSI(symbol) {
  let ok = false;
  let x;
  let stockNo = ssiSymbolMap[symbol];
  ...
  try {
    x = await a.json();
    // logger.info(x.data.leTables)
    for (var e of x.data.leTables) {
      e["symbol"] = symbol;
    }
    ok = true;
  } catch (err) {
...
async function getlistallstockssi() {
  let cop = [];
  ...
  for (let ex of exs) {
    ...
    for (var e of cop) {
      ssiSymbolMap[e.stockSymbol] = e.stockNo;
    }
    return cop;
  }
}
